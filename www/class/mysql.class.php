...
class cMySql
{
    ...
    private $db_host = DB_SERVER_IP; // server name
    private $db_user = DB_DATABASE_USER; // user name
    private $db_pass = DB_DATABASE_PASS; // password
    private $db_dbname = DB_DATABASE_NAME; // database name
     ...
     */
    static public function buildSQLWhereClause($aWhere)
    {
        $sWhere = "";
        foreach ($aWhere as $sKey => $mValue) {
            if(is_string($mValue)) $mValue = self::quoteStr($mValue);
            ...
            if (strlen($sWhere) == 0) {
                if (is_null($mValue)) {
                    $sWhere = " WHERE " . $sKey . " IS NULL";
                } else if (is_string($sKey)) {
                    $sWhere = " WHERE " . $sKey . " = " . $mValue;
                } else {
                    $sWhere = " WHERE " . $mValue;
                }
            } else {
                if (is_null($mValue)) {
                    $sWhere = " AND " . $sKey . " IS NULL";
                } else if (is_string($sKey)) {
                    $sWhere .= " AND " . $sKey . " = " . $mValue;
                } else {
                    $sWhere .= " AND " . $mValue;
                }
            }
    ...
    public function selectOne($tableName, $whereArray = null, $columns = null,
                              $sortColumns = null, $sortAscending = true,
                              $limit = null)
    {
        $this->resetError();
